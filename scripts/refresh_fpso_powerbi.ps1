...
function Write-PbiLog {
    ...
    Write-Host $line
    if ($script:PbiLogFile) {
        Add-Content -LiteralPath $script:PbiLogFile -Value $line -Encoding UTF8
    }
}
...
try {
    ...
    $script:PbiLogFile = Join-Path $dataset "FPSO_powerbi_last_refresh.log"

    Write-PbiLog "refresh_fpso_powerbi.ps1 starting (UnlockDataset=$UnlockDataset SkipRefresh=$SkipRefresh SkipPublish=$SkipPublish OpenOnly=$OpenOnly)"
    Write-PbiLog "workspace='$WorkspaceName' report=$reportName"

    if ($UnlockDataset) {
        $null = Close-PbiReport -ReportName $reportName
        Write-PbiLog "Dataset unlocked for harvest (Desktop closed for $reportName)."
        exit 0
    }
    ...
    if (Test-Path -LiteralPath $downloadsPbix) {
        if (-not (Test-Path -LiteralPath $stablePbix) -or
            ...
            Write-PbiLog "Updated stable PBIX from $downloadsPbix"
        }
        ...
        if (Test-Path -LiteralPath $clvPbix) {
            ...
            Write-PbiLog "Seeded $reportName.pbix from CLV template -> $stablePbix"
    ...
    Write-PbiLog "plan refresh=$doRefresh publish=$doPublish leaveOpen=$leave pbix=$stablePbix"
    ...
    if (Test-Path -LiteralPath $fpsoFact) {
        ...
        Write-PbiLog "Mirrored FPSO_wo_fact.csv -> CLV_wo_fact.csv (PBIX query name)."
    } else {
        throw "Fact CSV missing: $fpsoFact"
    }

    $result = @(Invoke-PbiRefreshAndPublish @params) | Select-Object -Last 1
    if (-not $result) {
        throw "Invoke-PbiRefreshAndPublish returned no result."
    }
    if ($doRefresh -and -not $result.Refreshed) {
        throw "Refresh did not start for $reportName (refreshed=False)."
    }
    Write-PbiLog ("Done. refreshed={0} published={1}" -f $result.Refreshed, $result.Published)
    exit 0
} catch {
    $msg = "FAIL: $($_.Exception.Message)"
    Write-Host $msg
    if ($script:PbiLogFile) {
        Add-Content -LiteralPath $script:PbiLogFile -Value ("[{0}] {1}" -f (Get-Date).ToString("s"), $msg) -Encoding UTF8
    }
    exit 1
}
